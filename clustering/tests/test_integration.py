...
BASE_URL = os.getenv("CLUSTERING_API_URL", "http://localhost:8000")


class TestIntegrationClustering:
    """Integration tests that require a running server"""

    def test_assign_routes_1(self):
        """
        ...
        test_data_1 = {
            "locations": {
                ...
                "employees": [
                    ...
                    {"id": "41ffa805-7b34-42ac-9418-891563aaa175", "latitude": 9.03369509386297, "longitude": 38.75478508606945},
                    ...
                    {"id": "0f52c71f-087e-42b7-935c-5f2eb047625c", "latitude": 8.987472802464548, "longitude": 38.7973012408324},
                    {"id": "81f3370c-71b8-4dc6-9995-0f26749a3fdd", "latitude": 8.98311777237035, "longitude": 38.77275682326565},
                    {"id": "0d5a7230-4857-4483-b861-bf4abd3e8b26", "latitude": 9.002150369296826, "longitude": 38.8423026262783},
                    {"id": "5cef5f61-47bf-4709-ae65-f73db238df7a", "latitude": 9.059905443974783, "longitude": 38.78734247206368},
                    {"id": "7c27a23f-61ee-4a7c-8e7f-81dcb0b130ca", "latitude": 9.042873786750272, "longitude": 38.76917973308376},
                    {"id": "f6b60409-fdfe-4006-bcb2-6b4369da62aa", "latitude": 9.00149837352932, "longitude": 38.78199238373926},
                    {"id": "3759680a-00e4-4917-b63d-eaf8fbee002b", "latitude": 9.002982911271694, "longitude": 38.79954123873755},
                    {"id": "80116ba1-3f86-4631-a5f9-7f2e7a8dc552", "latitude": 8.987445196580085, "longitude": 38.79727875809574},
                    {"id": "29d6adf1-6c17-4d4b-a759-6d2f4da38ecd", "latitude": 8.983138645898839, "longitude": 38.77279596649354},
                    {"id": "5f89c16c-2e86-4c19-92d8-e5ccc55f83de", "latitude": 8.987745877645528, "longitude": 38.78461777996745},
                    {"id": "9c06ba79-316f-4fd1-a371-a4843dbab330", "latitude": 8.99533589976133, "longitude": 38.80939259051409},
                    {"id": "502f8ad3-074b-4384-86ce-85e9389c9d09", "latitude": 8.9969024466439, "longitude": 38.83213045826451},
                    {"id": "5b38fd31-50de-42c5-99b4-c6a2f7931dc8", "latitude": 9.014863284775599, "longitude": 38.78388562634067},
                    {"id": "215c5c49-e490-4256-a345-aeb58191349d", "latitude": 9.019676093765247, "longitude": 38.80175503224659}
                ]
            },
            "shuttles": [
                {"id": 3, "capacity": 6},
        ...
        assert "total_demand" in response_data
        assert "total_capacity" in response_data

    def test_assign_routes_2(self):
        """
        Test the /clustering endpoint with the second dataset (same as first for now).
        """
        test_data_2 = {
            "locations": {
                "HQ": [9.0222, 38.7468],
    ...
    def test_assign_routes_multiple_datasets(self, test_data):
        """
        Test the /clustering endpoint with multiple datasets.
        """
        headers = {"Content-Type": "application/json"}
        response = requests.post(f"{BASE_URL}/clustering", data=json.dumps(test_data["data"]), headers=headers)

        assert response.status_code == 200

        response_data = response.json()
        print(f"{test_data['name']} Response: {json.dumps(response_data, indent=2)}")

        assert "routes" in response_data
        assert isinstance(response_data["routes"], list)
        assert len(response_data["routes"]) <= len(test_data["data"]["shuttles"])
        assert response_data["success"] is True
...
def check_server_running():
    """Check if the server is running before running integration tests"""
    try:
        response = requests.get(f"{BASE_URL}/health", timeout=5)
        if response.status_code != 200:
            pytest.skip(f"Server not responding at {BASE_URL}")
    except requests.exceptions.RequestException:
        pytest.skip(f"Cannot connect to server at {BASE_URL}. Start with: cd clustering && python -m uvicorn src.main:app --reload")